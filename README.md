...
Содержит в себе базовую логику для определение презентора. Имеет generic типы `T, K`. На вход конструктор принимает 3 параметра - событие `events: IEventEmitter`, модель `_model: T` и представление `_view: K`. В классе имплементированы следующие методы:

-   `get model(): T` - возвращает модель;
-   `get view(): K` - возвращает представление;
...
-   `set elements(value: HTMLElement[])` - установка элементов корзины;
-   `get elements(): HTMLElement[]` - возвращает элементы корзины;
...
Презентер класс корзины, наследуемый от общего класса `Presenter`, который будет использоваться для реагирования на события и изменять данные в модели `BasketModel` и оказывать влияние на `BasketView`

#### Класс OrderPresenter

Презентер класс заказа, наследуемый от общего класса `Presenter`, который будет использоваться для реагирования на события и изменять данные в модели `OrderModel` и оказывать влияние на `OrderView`

#### Класс CatalogPresenter

Презентер класс каталога, наследуемый от общего класса `Presenter`, который будет использоваться для реагирования на события и изменять данные в модели `CatalogModel` и оказывать влияние на `CatalogView`

### Общее

#### Класс WebLarekAPI

Класс представляет собой интерфейс для работы с запросами
